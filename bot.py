...
with st.sidebar:
    ...
    if uploaded_file_ui and "doc_ref" not in st.session_state:
        with st.spinner("Uploading document..."):
            ...
            with open("temp_doc", "wb") as f:#created a temp_doc file with writing capabilties
            ...
            st.session_state.chat = st.session_state.client.chats.create(
                ...
                config=types.GenerateContentConfig(
                    ...
                    "Answer questions OLY using the uploaded file."
                    "If the answer isn't there, say you don't know."
                )
            )
            st.success("Document uploaded successfully!")
        if "messages" not in st.session_state:
            st.session_state.messages = [] #create a list of dictionaries where each dictionary is a message containing role and content
